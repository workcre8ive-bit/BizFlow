    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6 md:space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-xs md:text-sm">Manage your client base and their contact information.</p>
        </div>
        ...
        <Card className="border-2 border-blue-100 bg-blue-50/30">
          <form onSubmit={handleAdd} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
            ...
            <div className="flex gap-2">
              ...
              <Button variant="ghost" type="button" className="flex-1" onClick={() => setShowAddForm(false)}>Cancel</Button>
            </div>
          </form>
        </Card>
      )}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left min-w-[800px] lg:min-w-0">
            <thead className="bg-slate-50 border-bottom border-slate-100">
              <tr>
                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-widest font-sans">Customer</th>
                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-widest font-sans">Contact</th>
                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-widest font-sans">Company</th>
                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-widest font-sans">Status</th>
                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-widest font-sans">Actions</th>
              </tr>
            </thead>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-400">Since {customer.createdAt}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
          ...
          {filteredCustomers.length === 0 && (
            <div className="py-12 md:py-20 text-center">
              ...
              <p className="text-slate-500 text-sm px-4">Try adjusting your search or add a new customer.</p>
            </div>
          )}
        </div>
      </Card>
