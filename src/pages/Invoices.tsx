    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">Create and manage professional invoices for your clients.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="flex items-center gap-4 border-l-4 border-l-blue-500">
          ...
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Total Issued</p>
            <p className="text-2xl font-bold text-slate-900">{formatCurrency(invoices.reduce((s, i) => s + i.amount, 0))}</p>
          </div>
        </Card>
        <Card className="flex items-center gap-4 border-l-4 border-l-emerald-500">
          ...
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Paid Amount</p>
            <p className="text-2xl font-bold text-slate-900">{formatCurrency(invoices.filter(i => i.status === 'paid').reduce((s, i) => s + i.amount, 0))}</p>
          </div>
        </Card>
      ...
      <AnimatePresence>
        {showAddForm && (
          ...
          >
            <Card className="border-2 border-blue-600 overflow-visible">
              <form onSubmit={handleAdd} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-500 uppercase">Select Customer</label>
                    <select 
                      className="w-full flex h-10 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-950"
                ...
                <div className="flex gap-3 justify-end">
                  <Button variant="ghost" type="button" onClick={() => setShowAddForm(false)}>Discard</Button>
      ...
      <Card className="p-0 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-slate-50 border-bottom border-slate-100 text-xs font-bold text-slate-500 uppercase tracking-widest">
              <tr>
          ...
          {filteredInvoices.length === 0 && (
            <div className="p-20 text-center">
              ...
              <p className="text-slate-500 text-sm">Create your first invoice to start getting paid.</p>
            </div>
          )}
        </div>
      </Card>
